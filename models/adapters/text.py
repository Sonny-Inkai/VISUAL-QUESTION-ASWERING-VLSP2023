...
def make_token_bucket_position(bucket_size, max_position):
    context_positon = torch.arange(max_position, dtype=torch.long)[:, None]
    memory_pos = torch.arange(max_position, dtype=torch.long)[:, None]
    relative_pos = context_positon - memory_pos
...
class TextAdapter(nn.Module):
    def __init__(self, config, embed_dim, attention_heads, src_dict=None, num_layers=None):
        super().__init__()
        self.dropout_module = FairseqDropout(
            config.dropout, module_name=self.__class__.__name__
        )
        self.alpha = config.shrink_alpha

        if src_dict is not None:
            num_embeddings = len(src_dict)
            padding_idx = src_dict.pad()
            self.embed_token = Embedding(embedding_dim=embed_dim, padding_idx=padding_idx, num_embeddings=num_embeddings)
        ...
        if config.layernorm_embedding:
            ...
            self.layerenorm_embedding = None
        ...
        self.embed_pos = Embedding(512+2, embed_dim)

        if config.use_attn_bias:
            num_rel_dis = 2 * config.bucket_size - 1
            rp_bucket = make_token_bucket_position(config.bucket_size, max_position=1024)
    ...
    def gather_features(self, adapter_embedding, pos_embed, self_attn_bias_list, position_ids):
        ...
        pos_embed = pos_embed.gather(1, pos_embed[:, :, None].expand(-1, -1, embed_dim))
    ...
    def forward(self, src_tokens, preserve_ids=None, preserve_embed=None, mask_token=None):
        ...
        if preserve_embed is not None:
            ...
            left_preserve_indices = preserve_ids + (torch.arange(bsz) * seq_len).unsequeeze(1).type_as(preserve_ids)
            ...
            adapter_embedding = self.embed_token(src_tokens)
            ...
            if preserve_ids is not None:
                ...
                adapter_embedding, pos_embed, self_attn_bias_list = self.gather_features(
                    adapter_embedding, pos_embed, self_attn_bias_list=self_attn_bias_list, position_ids=position_ids
                )
            if self.layernorm_embedding is not None:
                adapter_embedding = self.layerenorm_embedding(adapter_embedding)
            if self.alpha != 1.0:
    ...
    def upgrade_state_dict_named(self, state_dict, name):
        ...
        and prefix + 'rel_pos_table.1.weight' not in state_dict:
